<template>
  <div class="MovieList__wrapper">
    <transition name="fade">
      ...
      <div
        v-else-if="!loading && movieList.length === 0"
        class="MovieList__empty"
      >
        {{ emptyTitle }}
      </div>
    </transition>
    <transition name="fade">
      <div
        v-if="selectedMovie"
        class="MovieList__details-backdrop"
        @click="unselectMovie"
      >
    ...
    loadMovies() {
      ...
      axios
        ...
        .then((response) => {
          ...
          this.totalPages =
            response.data.total_pages < 1000 ? response.data.total_pages : 1000;
        })
        .then(() => {
          window.scrollTo(0, 0);
        })
